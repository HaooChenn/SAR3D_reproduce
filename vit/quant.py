...
# This file provides the VectorQuantizer2 class used in VQVAE
__all__ = ['VectorQuantizer2',]


class VectorQuantizer2(nn.Module):
    """Vector quantizer with multi-scale codebook and residual quantization.
    
    Args:
        vocab_size: Size of the codebook
        Cvae: Dimension of codebook embeddings
        using_znorm: Whether to normalize embeddings
        beta: Commitment loss coefficient (default: 0.25)
        default_qresi_counts: Number of residual quantizers
        v_patch_nums: List of patch sizes for multi-scale quantization
        quant_resi: Residual quantization ratio
        share_quant_resi: How to share residual quantizers (0: non-shared, 1: fully shared, >1: partially shared)
    """
    def __init__(
        ...
        # Initialize residual quantizers based on sharing strategy
        if share_quant_resi == 0:   # Non-shared: separate quantizer for each scale
        ...
        elif share_quant_resi == 1: # Fully shared: single quantizer for all scales  
            self.quant_resi = PhiShared(Phi(Cvae, quant_resi) if abs(quant_resi) > 1e-6 else nn.Identity())
        else:                       # Partially shared: share quantizers among subsets of scales
        ...
        # For progressive training (not currently supported)
        self.prog_si = -1
    
    def eini(self, eini):
        """Initialize embedding weights with specified standard deviation"""
        if eini > 0: 
    ...
    def forward(self, f_BChw: torch.Tensor, ret_usages=False) -> Tuple[torch.Tensor, List[float], torch.Tensor]:
        ...
        Args:
            f_BChw: Input features [B,C,H,W]
            ret_usages: Whether to return codebook usage statistics
            
        Returns:
            f_hat: Quantized features
            usages: Codebook usage statistics (if ret_usages=True)
            mean_vq_loss: Vector quantization loss
        """
        dtype = f_BChw.dtype
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            for si, pn in enumerate(self.v_patch_nums):
                assert self.using_znorm, 'Only using znorm is supported'
                ...
                if self.using_znorm:
                    ...
                    idx_N = torch.argmax(rest_NC @ F.normalize(embedding.data.T, dim=0), dim=1)
                else:
                    rest_NC = F.interpolate(f_rest, size=(pn, pn), mode='area').permute(0, 2, 3, 1).reshape(-1, C) if (si != SN-1) else f_rest.permute(0, 2, 3, 1).reshape(-1, C)
                ...
                h_BChw = F.interpolate(embedding[idx_Bhw].permute(0, 3, 1, 2), size=(H, W), mode='bicubic').contiguous() if (si != SN-1) else self.embedding(idx_Bhw).permute(0, 3, 1, 2).contiguous()
        ...
        margin = [f_BChw.shape[0] * pn * pn / self.vocab_size * 0.01 for si, pn in enumerate(self.v_patch_nums)]
        if ret_usages:
            usages = [(self.ema_vocab_hit_SV[si] > margin[si]).float().mean().item() * 100 for si, pn in enumerate(self.v_patch_nums)]
        else:
            usages = None
    ...
    def f_to_idxBl_or_fhat(self, f_BChw: torch.Tensor, to_fhat: bool, v_patch_nums: Optional[Sequence[Union[int, Tuple[int, int]]]] = None) -> List[Union[torch.Tensor, torch.LongTensor]]:  # z_BChw is feature from inp_img_no_grad
        B, C, H, W = f_BChw.shape
        f_no_grad = f_BChw.detach()
        f_rest = f_no_grad.clone()
        f_hat = torch.zeros_like(f_rest)
    ...
    def idxBl_to_var_input(self, gt_ms_idx_Bl: List[torch.Tensor]) -> torch.Tensor:
        ...
        # Refer to llamaGen
        assert self.using_znorm == True, 'VAR training should always use znorm'
        embedding = F.normalize(self.embedding.weight, p=2, dim=-1)
        for si in range(SN-1):
            if self.prog_si == 0 or (0 <= self.prog_si-1 < si): break   # Progressive training not supported yet, prog_si always -1
            h_BChw = F.interpolate(embedding[gt_ms_idx_Bl[si]].transpose_(1, 2).view(B, C, pn_next, pn_next), size=(H, W), mode='bicubic')
