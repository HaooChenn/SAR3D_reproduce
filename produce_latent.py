...
def save_latent(ld_or_itrt, args: arg_util.Args, trainer, dataset_size):
    ...
    dino_image_processor = AutoImageProcessor.from_pretrained("facebook/dinov2-large")
    ...
    for iter, data in enumerate(tqdm(ld_or_itrt), start=1):
        ...
        #     # st()
        #     np.save( os.path.join(data['sample_path'][sample_index], "gt_BL_dim_32.npy"), gt_BL[sample_index].cpu().detach().numpy())
        #     np.save( os.path.join(data['sample_path'][sample_index], "x_BLCv_wo_first_l_dim_32.npy"), x_BLCv_wo_first_l[sample_index].cpu().detach().numpy())
        ...
        for idx in range(pooler_output.shape[0]):
            ...
            np.save( os.path.join(data['sample_path'][idx], "image_dino_embedding.npy"), dino_embedding[idx].cpu().numpy())
            np.save( os.path.join(data['sample_path'][idx], "image_dino_pooler_output.npy"), dino_pooler_output[idx].cpu().numpy())
