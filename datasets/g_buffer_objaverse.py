...
class ChunkObjaverseDataset(Dataset):
    def __init__(
        ...
        # indicate if it should load the whole data
        ...
        for cl in [  # ! around 17W instances in total. 
            ...
            with open(f'{self.file_path}/dataset.json', 'r') as f:
                # dataset_json = json.load(f)['Animals'][35703:]
                cl_dataset_json = json.load(f)[cl][::4]
                # st()
                # dataset_json = json.load(f)['Animals'][:25000]
    ...
    def load_latent(self, sample, latent_path=None):
        "load latent from pth file"
        # gt_BL = torch.from_numpy(np.load(os.path.join(latent_path, "gt_BL_dim_8_l2norm_lrm.npy")))
        # x_BLCv_wo_first_l = torch.from_numpy(np.load(os.path.join(latent_path, "x_BLCv_wo_first_l_dim_8_l2_norm_lrm.npy")))
        ...
        # 'text_pooler_output': pooler_ouput
        # 'image_clip_embedding': image_clip_embedding,
        # 'image_clip_pooler_output': image_clip_pooler_output,
        ...
        #     })
        # else:
        #     raise NotImplementedError(os.path.join(latent_path, "gt_BL_dim_8_l2norm_lrm.npy"))
            

        
        # print(image_dino_pooler_output.shape)
        # print(torch.from_numpy(np.load(os.path.join(latent_path, "image_clip_pooler_output.npy"))).shape)
        # st()

        # gt_BL = torch.load(os.path.join(latent_path, "gt_BL.pth"))
        # x_BLCv_wo_first_l = torch.load(os.path.join(latent_path, "x_BLCv_wo_first_l.pth"))
        # text_embedding = torch.load(os.path.join(latent_path, "text_embedding.pth"))
        # pooler_ouput = torch.load(os.path.join(latent_path, "text_pooler_output.pth"))
        # image_clip_embedding = torch.load(os.path.join(latent_path, "image_clip_embedding.pth"))
        # image_clip_pooler_output = torch.load(os.path.join(latent_path, "image_clip_pooler_output.pth"))
        # image_dino_embedding = torch.load(os.path.join(latent_path, "image_dino_embedding.pth"))[1:, :]
    ...
    def __getitem__(self, index) -> Any:
        ...
        # print(os.path.join(self.file_path, self.chunk_list[index]))
        # st()
        # gt_BL = torch.load(os.path.join(os.path.join(self.file_path, self.chunk_list[index]), "gt_BL.pth"))
        # x_BLCv_wo_first_l = torch.load(os.path.join(os.path.join(self.file_path, self.chunk_list[index]), "x_BLCv_wo_first_l.pth"))
...
class ChunkObjaverseDataset_eval(Dataset):
    ...
    def load_latent(self, sample, latent_path=None):
        "load latent from pth file"
        # gt_BL = torch.from_numpy(np.load(os.path.join(latent_path, "gt_BL_dim_32.npy")))
        # x_BLCv_wo_first_l = torch.from_numpy(np.load(os.path.join(latent_path, "x_BLCv_wo_first_l_dim_32.npy")))
        # gt_BL = torch.from_numpy(np.load(os.path.join(latent_path, "gt_BL_dim_8_l2norm.npy")))
        # x_BLCv_wo_first_l = torch.from_numpy(np.load(os.path.join(latent_path, "x_BLCv_wo_first_l_dim_8_l2_norm.npy")))
        # image_clip_embedding = torch.from_numpy(np.load(os.path.join(latent_path, "image_clip_embedding.npy")))
        # image_clip_pooler_output = torch.from_numpy(np.load(os.path.join(latent_path, "image_clip_pooler_output.npy")))
        # image_dino_embedding = torch.from_numpy(np.load(os.path.join(latent_path, "image_dino_embedding.npy")))[1:, :]
        if os.path.exists(os.path.join(latent_path, "gt_BL_dim_8_l2norm_lrm.npy")):
            # gt_BL = torch.from_numpy(np.load(os.path.join(latent_path, "gt_BL_dim_8_l2norm_lrm.npy")))
            # x_BLCv_wo_first_l = torch.from_numpy(np.load(os.path.join(latent_path, "x_BLCv_wo_first_l_dim_8_l2_norm_lrm.npy")))
            gt_BL = torch.from_numpy(np.load(os.path.join(latent_path, "gt_BL_dim_8_l2norm_lrm_256.npy")))
