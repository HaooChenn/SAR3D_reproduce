...
from .utils import *
from .g_buffer_objaverse import load_data_3D_VAR

def make_object_point_data_module(tokenizer: transformers.PreTrainedTokenizer, data_args):
...
def make_object_point_data_module_objaverse(tokenizer: transformers.PreTrainedTokenizer, data_args):
    """Make dataset and collator for Joint3Ddataset with text and point cloud data."""
    """Initialize datasets."""

    # data_collator = DataCollatorForPointTextDataset(tokenizer=tokenizer)
    data_collator = DataCollatorForPointTextDatasetVAR(tokenizer=tokenizer)
    print(data_args.conversation_types, "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
    if data_args.split_train_val:
        print("Loading training datasets.")
        # train_dataset = ObjectPointCloudDataset(
        #     split='train',
        ...
        # )
        st()
        train_dataset = load_data_3D_VAR(file_path=data_args.data_path,
        ...
        if data_args.data_debug_num > 0:
            ...
            #     data_args=data_args
            # )
            val_dataset = load_data_3D_VAR(file_path=data_args.data_path,
                batch_size=data_args.per_device_train_batch_size,
                conversation_types=data_args.conversation_types,
                reso=256,
                reso_encoder=256,  # 224 -> 128
                num_workers=3,
                load_depth=True,
