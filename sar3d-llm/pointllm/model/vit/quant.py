...
from pointllm.model.vit import dist
    ...
    def forward(self, f_BChw: torch.Tensor, ret_usages=False) -> Tuple[torch.Tensor, List[float], torch.Tensor]:
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            for si, pn in enumerate(self.v_patch_nums): # from small to large
                ...
                # using znorm or Eucledian distance to find the nearest embedding
                # True here
                ...
                # st()
                # minlength represents the number of bins
                ...
                # f_hat 第一层是全0的（H, W)，然后每次加上一个h_BChw, 其中h_BChw是上一层的残差的quantization过一个网络，即\phi(quantization)
                # 即最后的f_hat是所有层的\phi(quantization)的和
                f_hat = f_hat + h_BChw
                # f_rest's shape is (H, W), and it is the resisule after quantization
                f_rest -= h_BChw
                # st()
                if self.training and dist.initialized():
                    handler.wait()
                    # print("record_hit: ", self.record_hit)
                ...
                # here they use f_hat.data rather than f_hat, because they need to stop gradient of f_hat
                mean_vq_loss += F.mse_loss(f_hat.data, f_BChw).mul_(self.beta) + F.mse_loss(f_hat, f_no_grad)
            
            mean_vq_loss *= 1. / SN
            # To stop graient of f_hat, we need to use f_hat.data
            f_hat = (f_hat.data - f_no_grad).add_(f_BChw)
        # st()
        # margin = tdist.get_world_size() * (f_BChw.numel() / f_BChw.shape[1]) / self.vocab_size * 0.08

        # margin = [ f_BChw.shape[0] * pn * pn / self.vocab_size * 0.08 for si, pn in enumerate(self.v_patch_nums)]
        margin = [ f_BChw.shape[0] * pn * pn / self.vocab_size * 0.01 for si, pn in enumerate(self.v_patch_nums)]
        # print(margin)
        # st()
        # margin = pn*pn / 100
        # self.ema_vocab_hit_SV: 每个word被使用的加权次数
        # st()
        # total usages
        # if ret_usages:
        #     total_hit =  [(self.ema_vocab_hit_SV[si] > margin[si]) for si, pn in enumerate(self.v_patch_nums)]
        #     usages = total_hit[0] 
        #     for i in range(1, len(total_hit)):
        #         usages += total_hit[i]
        # usages = usages.float().mean().item() * 100
        # print(usages)
        if ret_usages: usages = [(self.ema_vocab_hit_SV[si] > margin[si]).float().mean().item() * 100 for si, pn in enumerate(self.v_patch_nums)]
        else: usages = None
