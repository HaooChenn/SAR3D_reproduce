...
class Encoder(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
        ...
        # double_z == True here, but  we need to set it False
        if add_fusion_layer: # fusion 4 frames
            self.fusion_layer = torch.nn.Conv2d(2*z_channels*6 if double_z else z_channels*6,
                                            2*z_channels if double_z else z_channels,
                                            kernel_size=3,
                                            stride=1,
                                            padding=1)
...
class MVEncoder(Encoder):
    ...
    def forward(self, x):
        ...
        #     shuffle_h.append(h[i][indices])
        # h = shuffle_h
            # h[i] = torch.cat([h[i][j] for j in indices], 0)
        # shuffle_h = torch.cat(shuffle_h, 0)
