...
from utils.torch_utils.ops import conv2d_resample, upfirdn2d, bias_act, fma
...
def modulated_conv2d(
    x, weight, styles, noise=None, up=1, down=1, padding=0,
    resample_filter=None, demodulate=True, flip_weight=True, fused_modconv=True
):
    ...
    if demodulate or fused_modconv:
        w = weight.unsqueeze(0) * styles.reshape(batch_size, 1, -1, 1, 1)
    if demodulate:
...
class Conv2dLayer(torch.nn.Module):
    ...
    def extra_repr(self):
        return f'in_channels={self.in_channels}, out_channels={self.out_channels}, activation={self.activation}, up={self.up}, down={self.down}'
...
class MappingNetwork(torch.nn.Module):
    ...
    def extra_repr(self):
        return f'z_dim={self.z_dim}, c_dim={self.c_dim}, w_dim={self.w_dim}, num_ws={self.num_ws}'
...
class SynthesisNetwork(torch.nn.Module):
    ...
    def extra_repr(self):
        return f'w_dim={self.w_dim}, num_ws={self.num_ws}, img_resolution={self.img_resolution}, img_channels={self.img_channels}, num_fp16_res={self.num_fp16_res}'
...
class Discriminator(torch.nn.Module):
    def __init__(self, c_dim, img_resolution, img_channels, architecture='resnet', channel_base=32768, channel_max=512, num_fp16_res=4, conv_clamp=256, cmap_dim=None, block_kwargs={}, mapping_kwargs={}, epilogue_kwargs={}):
        ...
        cmap_dim = channels_dict[4] if cmap_dim is None else cmap_dim
        cmap_dim = 0 if c_dim == 0 else cmap_dim
    ...
    def extra_repr(self):
        return f'c_dim={self.c_dim}, img_resolution={self.img_resolution}, img_channels={self.img_channels}'
