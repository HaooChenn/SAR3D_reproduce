...
def compute_vertex_normal(v_pos, t_pos_idx):
    ...
    face_normals = torch.cross(v1.float() - v0.float(), v2.float() - v0.float())
    face_normals = face_normals.to(v0.dtype)
    ...
    def render_mesh(
        ...
        with dr.DepthPeeler(self.ctx, v_pos_clip.float(), mesh_t_pos_idx_fx3, [resolution * spp, resolution * spp]) as peeler:
            for _ in range(num_layers):
                ...
                # import ipdb; ipdb.set_trace()
                gb_feat, _ = interpolate(mesh_v_feat_bxnxd.float(), rast, mesh_t_pos_idx_fx3)
                gb_feat = gb_feat.to(v_pos_clip.dtype)
        ...
        antialias_mask = dr.antialias(
            ...
            mesh_t_pos_idx_fx3)
        antialias_mask = antialias_mask.to(v_pos_clip.dtype)
        ...
        # import ipdb; ipdb.set_trace()
        normal = dr.antialias(normal.clone().contiguous(), rast, v_pos_clip.float(), mesh_t_pos_idx_fx3)
        normal = F.normalize(normal, dim=-1)
        normal = torch.lerp(torch.zeros_like(normal), (normal + 1.0) / 2.0, hard_mask.float())      # black background

        normal = normal.to(v_pos_clip.dtype)
